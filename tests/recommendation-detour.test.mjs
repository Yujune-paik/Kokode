...
test("大手町のような巨大駅は集合候補生成から除外する", () => {
  const path = ["外苑前", "赤坂見附", "大手町", "飯田橋", "小竹向原", "練馬", "練馬高野台"];
  const candidates = path.filter((station) => !isLargeTerminalStation(station));

  assert.equal(candidates.includes("大手町"), false);
  assert.equal(candidates.includes("飯田橋"), true);
  assert.equal(candidates.includes("小竹向原"), true);
  assert.equal(candidates.includes("練馬"), true);
});
